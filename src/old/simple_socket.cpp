...
//	t1 = new std::thread(&simple_socket::thread_func, this);
}
...
void simple_socket::send(const uchar* data, ulong bytes) {
	//auto wcts = std::chrono::system_clock::now();
	long write_bytes;
	ulong offset = 0L;
	while (offset < bytes) {
...
void simple_socket::recv(uchar* data, ulong bytes) {
	long read_bytes;
	ulong offset = 0L;
	auto wcts = std::chrono::system_clock::now();
	while (offset < bytes) {
...
void simple_socket::thread_func() {
	ulong bytes;
	while (flag) {
		//std::cout << "IIIIIIIIIIIIIIIIIIIII" << std::endl;
		recv((uchar*) &bytes, 8);
		uchar* buff = new uchar[bytes];
		recv(buff, bytes);
		bq.push(buff);
	}
}
